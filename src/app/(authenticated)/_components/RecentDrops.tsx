...
export type ItemType = RouterOutput["items"]["getLatestDrops"][number]

interface RecentDropsFragmentProps {
  item: ItemType
}

function RecentDropsFragment(props: RecentDropsFragmentProps) {
  return (
    <li className="flex items-center gap-2 px-0.5 py-2.5 text-sm first-of-type:pt-0 last-of-type:pb-0">
      ...
      <div className="ml-auto flex flex-col gap-2 text-end leading-none">
        ...
        <p className="font-semibold">{props.item.acquired_by}</p>
      </div>
    </li>
  )
}
